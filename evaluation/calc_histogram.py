...
def make_histogram(cond_name, measure, coord_dir, out_dir, visualize=True, width=1):
    ...
    return predicted_total, bins


def main():
    parser = argparse.ArgumentParser(
        description='Calculate histograms of moving distances')
    parser.add_argument('--coords_dir', '-c', default='../my_inference/bvh/bvh2npy/',
    ...
    parser.add_argument('--out_dir', default='resulting_hmd_csv',
    ...
    for cond_name in os.listdir(args.coords_dir):
        if cond_name == ".DS_Store":
            continue
        print("\nConsider", cond_name)
        predicted_total_cur, bin = make_histogram(cond_name, args.measure, args.coords_dir, args.out_dir,
                                                  args.visualize, args.width)
        # predicted_total.append(predicted_total)
        sum = np.sum(predicted_total_cur)
        actual_freq = predicted_total_cur / sum
        plt.plot(bin[:-1], actual_freq, label=cond_name)

    if args.measure == "acceleration":
        plt.legend()
        plt.xlabel('Acceleration (cm/s^2)')
        plt.ylabel('Frequency')
        # plt.title('Frequencies of Moving Distance ({})'.format(measure))
        # plt.title('Frequencies of Moving Distance (Acceleration)')
        plt.tight_layout()
        plt.show()
    elif args.measure == "velocity":
        plt.legend()
        plt.xlabel('Velocity (cm/s)')
        plt.ylabel('Frequency')
        # plt.title('Frequencies of Moving Distance ({})'.format(measure))
        # plt.title('Frequencies of Moving Distance (Velocity)')
        plt.tight_layout()
        plt.show()
...
if __name__ == '__main__':
    # python calc_histogram.py -c ../my_inference/bvh2npy/ -m velocity -w 0.05 -v
    main()
